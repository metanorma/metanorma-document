...
=== Reverse Conversion (Mirror → Metanorma)

The reverse converter rebuilds a Model graph, applying skip rules and optional per-type customization:

[source,ruby]
reverse = Metanorma::Mirror::MirrorToMetanorma.new
rebuilt = reverse.call(document)

Customization is OCP-friendly:

[source,ruby]
Metanorma::Mirror::MirrorToMetanorma.skip("my_type")
Metanorma::Mirror::MirrorToMetanorma.register("my_type") do |node, ctx|
...
[source,ruby]
Metanorma::Mirror::Output::HtmlRenderer.register_node_renderer("callout",
  ->(node, renderer) { "<callout>#{renderer.e(node.attrs['text'])}</callout>" })

Metanorma::Mirror::Output::HtmlRenderer.register_mark_renderer("kbd",
  ->(text, _mark) { "<kbd>#{text}</kbd>" })
