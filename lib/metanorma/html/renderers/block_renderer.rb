      ...
      class BlockRenderer
        ...
        def render_paragraph(p, **_opts)
          attrs = element_attrs(id: safe_attr(p, :id),
                                style: alignment_style(safe_attr(p, :alignment)))
          content = coordinator.inline_renderer.render_mixed_inline(p)
        ...
        def render_table(table, **_opts)
          ...
          if table_id && name_el
            register_table_entry(id: table_id, text: coordinator.extract_plain_text(name_el))
          end
          col_count = table_column_count(table)

          caption_html = if name_el
                           coordinator.inline_renderer.render_inline_element(name_el)
                         end
        ...
        def render_table_colgroup(colgroup)
          colgroup.col&.filter_map do |col|
            ...
            render_liquid("_element.html.liquid", "tag" => "col", "extra_attrs" => attrs, "content" => "")
          end.join
        end

        def render_table_section(section, tag_name)
          inner = render_table_section_rows(section)
          render_liquid("_element.html.liquid", "tag" => tag_name, "extra_attrs" => "", "content" => inner)
        end

        def render_table_section_rows(section)
          section.tr&.filter_map do |tr|
            inner_parts = []
            walked = coordinator.inline_renderer.walk_ordered(tr) do |type, obj|
              next unless type == :element

              inner_parts << (render_table_cell(obj) || "")
            end
            unless walked
              Array(tr.th).each { |th| inner_parts << (render_table_cell(th, "th") || "") }
              Array(tr.td).each { |td| inner_parts << (render_table_cell(td, "td") || "") }
            end
            render_liquid("_element.html.liquid", "tag" => "tr", "extra_attrs" => "", "content" => inner_parts.join)
          end.join
        end

        def render_unordered_list(ul, **_opts)
          attrs = element_attrs(id: safe_attr(ul, :id))
          items = ul.listitem&.filter_map { |li| render_list_item_content(li) } || []
        ...
        def render_table_cell(cell, force_tag = nil)
          ...
          content = coordinator.inline_renderer.render_cell_content(cell)
        ...
        def render_ordered_list(ol, **_opts)
          ...
          items = ol.listitem&.filter_map { |li| render_list_item_content(li) } || []
          render_liquid("_list.html.liquid", {
                          "list_tag" => "ol",
                          "attrs" => attrs,
                          "items" => items,
        ...
        def render_list_item_content(li)
          ...
          inner = coordinator.inline_renderer.render_mixed_content_in_order(li)
          render_liquid("_list_item.html.liquid", "attrs" => attrs, "content" => inner)
        end

        def render_definition_list(dl, **_opts)
          attrs = element_attrs(id: safe_attr(dl, :id))
          terms = dl.dt&.each_with_index&.map do |dt, i|
            dt_html = coordinator.inline_renderer.render_mixed_inline(dt)
            dd = dl.dd&.[](i)
            dd_html = dd ? coordinator.inline_renderer.render_mixed_inline(dd) : nil
        ...
        def render_figure(figure, **_opts)
          drop = Drops::FigureDrop.from_model(figure, renderer: coordinator.renderer_context)
        ...
        def render_note(note, **_opts)
          drop = Drops::NoteDrop.from_model(note, renderer: coordinator.renderer_context)
          render_liquid("_note.html.liquid", { "block" => drop })
        end

        def render_example(example, **_opts)
          drop = Drops::ExampleDrop.from_model(example, renderer: coordinator.renderer_context)
          render_liquid("_example.html.liquid", { "block" => drop })
        end

        def render_sourcecode(sc, **_opts)
          drop = Drops::SourcecodeDrop.from_model(sc, renderer: coordinator.renderer_context)
          render_liquid("_sourcecode.html.liquid", { "block" => drop })
        end

        def render_formula(formula, **_opts)
          drop = Drops::FormulaDrop.from_model(formula, renderer: coordinator.renderer_context)
        ...
        def render_quote(quote, **_opts)
          ...
          content_parts = []
          quote.paragraphs&.each { |para| content_parts << (render_paragraph(para) || "") }
          quote.ul&.each { |ul| content_parts << (render_unordered_list(ul) || "") }
          quote.ol&.each { |ol| content_parts << (render_ordered_list(ol) || "") }
          content = content_parts.join
          attribution_html = if quote.attribution
                               coordinator.inline_renderer.render_mixed_inline(quote.attribution)
                             end
        ...
        def render_admonition(admonition, **_opts)
          drop = Drops::AdmonitionDrop.from_model(admonition, renderer: coordinator.renderer_context)
        ...
        def render_block_children(model, children:)
          ...
          children.each do |attr, render_method|
            ...
            Array(values).each { |v| parts << (public_send(render_method, v) || "") }
          end
          parts.join
        end
